...
async def proxy_by_source(source_url: str):
    async with httpx.AsyncClient(timeout=30, follow_redirects=True) as client:
        r = await client.get(
            f"{ECONOMY_URL}/jobs/by-source",
            params={"source_url": source_url},
        )
    logger.info(f"by-source: status={r.status_code} hist={[str(h.url) for h in r.history]}")
    if r.text:
        try:
            return r.json(), r.status_code
        except Exception as e:
            logger.error(f"by-source parse error: {e}")
