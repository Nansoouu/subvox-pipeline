...
async def step_download(
    ...
    if not existing_source:
        ...
        dl_cmd = base_cmd + ["--no-part", "--retries", "5", "-f", "bestvideo[height<=1080][ext=mp4]+(bestaudio[ext=m4a]/bestaudio)/best[height<=1080][ext=mp4]/best", "--merge-output-format", "mp4", "--no-playlist", "-o", str(source_mp4), source_url]
