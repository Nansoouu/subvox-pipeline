...
<p align="center">
  <strong>オープンソースの動画翻訳エンジン。</strong><br/>
  ダウンロード。文字起こし。翻訳。字幕焼き込み。ひとつのパイプラインで、あらゆる言語に対応。
</p>
...
> 私は35歳です。10年間、事業を経営していました。その後、テクノロジーに出会いました。
>
> それはキャリアの転換ではありませんでした。啓示でした。自分のターミナルから、国境を越えるツールを構築できる力。
>
> Subvoxはこの探究心から生まれました。韓国の政治演説、ドイツ語のチュートリアル、アラビア語のカンファレンス — どんな動画でも、数分で母国語でアクセス可能にし、声、トーン、意図を失わずに済むとしたら？
>
> Subvoxはまた、コミュニティとして貢献し、動画翻訳をすべての人が利用できるようにすることを目指しています。**Veed.io**、**Kapwing**、**Descript**、**Opus Clip** のような巨人たちとは異なり、無料であるべきものに法外な料金を請求し、エクスポートを制限し、サブスクリプションに縛り付けます。Subvoxはオープンで透明性があり、それを使う人々によって構築されています。
>
> これは一人のプロジェクトであり、コミットごとに積み上げられています。なぜなら、テクノロジーはすべての人のものであるべきだと信じているからです。英語を話す人だけのものではありません。
>
> このパイプラインは技術的な核です。残り（ウォレット、トークン、エコノミー）は別の場所にあります。ここでは、世界を翻訳する機械があります。無料。オープン。あなたのもの。
>
> *Nansou*
...
| ステップ | 説明 |
|---|---|
| **⬇️ ダウンロード** | X/Twitter、YouTube、または直接URLから動画を取得 |
| **🎙️ 文字起こし** | Groq（Whisper）による音声文字起こし、20以上の言語に対応 |
| **🌐 翻訳** | LLM翻訳（DeepSeek / OpenAI）でターゲット言語へ |
| **🎬 字幕焼き込み** | ffmpeg/libassで動画に字幕を合成 |
| **☁️ アップロード** | 最終結果のローカルまたはS3ストレージ |

**ボーナス:** VTT/SRTエクスポート、自動ウォーターマーク、シーン検出、コンテンツ分析。
...
1. 動画リンクを貼り付け
   ...
   │  バリデーターが2秒でアクセス可能性をチェック
   ▼
2. ダウンロード
   │  yt-dlpが動画を取得（最大4K）
   ▼
3. 音声文字起こし
   │  Groq Whisperが音声をテキストに変換
   │  20以上の言語に対応
   ▼
4. 翻訳
   │  DeepSeek / OpenAIが字幕を翻訳
   │  あなたが選んだ言語で
   ▼
5. 字幕焼き込み
   │  ffmpeg + libassが動画に字幕を焼き込み
   │  自動ウォーターマーク、カスタマイズ可能なスタイル
   ▼
6. 結果
   │  字幕付き動画、共有準備完了
   │  SRT/VTTエクスポート対応
...
**合計時間:** 3分の動画で2〜5分。
**コーディング不要。** リンクを貼り、言語を選び、翻訳済み動画を入手。
...
**Pipeline** = 純粋な技術（このリポジトリ）。認証のためにHTTP経由で **Economy** を呼び出します。
...
| ルート | メソッド | 説明 |
|---|---|---|
| `/jobs/feed` | GET | 最新の公開翻訳 |
| `/jobs/{id}/status` | GET | ジョブのステータス |
| `/jobs/{id}/subtitles` | GET | 生成された字幕 |
| `/health` | GET | ヘルスチェック |

認証とトークン管理は専用サービス（非公開）によって処理されます。
...
| 動画 | FFmpeg 7 |
| 文字起こし | Groq（Whisper） |
> **注:** Groqは1日あたり2時間の無料文字起こしを提供しています。開発や小規模プロジェクトに最適です。
| 翻訳 | DeepSeek / OpenAI |
| DB | PostgreSQL 16 |
...
このプロジェクトはまだ若く、一人の人物によって構築されています。すべての貢献を歓迎します：

- **Issues**: バグの報告、機能の提案
- **PRs**: コード、ドキュメント、テスト — すべてが役立ちます
- **Discussions**: ユースケースを共有してください

パイプラインは **100% オープンソース**（MIT）です。エコノミーレイヤー（ウォレット、トークン）はセキュリティ上の理由から非公開のままです。

---

## 🙏 謝辞

Subvox Pipelineは以下の重要なオープンソースプロジェクトに依存しています：
...
MIT、[Nansou](https://github.com/Nansouoouu) が ❤️ を込めて制作
